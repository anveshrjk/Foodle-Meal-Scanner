...
export default function SearchPage() {
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      ...
      const results = mockFoodDatabase.filter((food) => food.name.toLowerCase().includes(searchQuery.toLowerCase()))
  ...
  const analyzeFood = useCallback(
    async (food: (typeof mockFoodDatabase)[0]) => {
      setIsAnalyzing(true)
      setError(null)
      ...
      try {
        ...
        const isRecommended = food.calories < 300 && food.protein > 5

        const mockRecommendation = {
          is_recommended: isRecommended,
          reason: isRecommended
            ? `${food.name} is a great choice! It's nutritious and aligns with your health goals.`
            : `${food.name} might not be the best choice right now. Consider portion control or alternatives.`,
          tips: [
        ...
        router.push(`/scan/results?food=${encodeURIComponent(food.name)}&recommended=${isRecommended}`)
      } catch (err) {
        ...
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center space-x-2">
              ...
              <h1 className="text-2xl font-bold text-emerald-800">Food Search</h1>
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="container mx-auto px-6 py-8 max-w-4xl">
        <Card className="border-emerald-200">
          <CardHeader className="text-center">
            <CardTitle className="text-emerald-800">Search Food Database</CardTitle>
          ...
          <CardContent className="space-y-6">
            ...
            {searchResults.length > 0 && (
              <div className="space-y-4">
                <h3 className="text-lg font-semibold text-emerald-800">Search Results</h3>
                      ...
                      <CardContent className="p-4">
                        <div className="flex items-center justify-between">
                          <div className="flex-1">
                            <h4 className="font-semibold text-emerald-800 mb-2">{food.name}</h4>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm text-emerald-600">
                              <div>
            ...
            {searchQuery && searchResults.length === 0 && !isSearching && (
              <div className="text-center py-8">
                <p className="text-emerald-600">No foods found matching "{searchQuery}"</p>
                <p className="text-sm text-emerald-500 mt-2">
                  Try searching for common foods like fruits, vegetables, or proteins
                </p>
              </div>
            )}
          </CardContent>
        ...
        <Card className="mt-6 border-emerald-200 bg-emerald-50">
          <CardHeader>
            <CardTitle className="text-emerald-800">Popular Foods</CardTitle>
            <CardDescription className="text-emerald-600">Quick access to commonly searched foods</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="flex flex-wrap gap-2">
              {["Apple", "Banana", "Chicken Breast", "Brown Rice", "Broccoli", "Salmon"].map((food) => (
                <Button
                  key={food}
                  variant="outline"
                  size="sm"
